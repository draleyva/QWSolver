...
let FoodRequestPresentation = class {
  constructor()
  {
    this.food = null;
    this.name = '';
...
let Modular = class {
  ...
  async performRequest(caretype, delivery, foodpresentationarray){
    if(!this.careTypeMap.has(caretype))
      throw new Error('Invalid caretype');
  ...
  async performDelivery(caretype, delivery, foodpresentationarray){
    ...
    for (const [key, value] of deliveryitem.foodDeliveryMap.entries()) {
      ...
      //if(foodshoppingitem.food.name == 'LECHE EVAPORADA ENTERA')
...
let Item = class {
  ...
  async performBuildPackage(caretype, delivery, foodpresentationarray) {
    ...
    for (const [key, value] of this.modularMap.entries()) {
      ...
      packagedetail.moduleArray.push(key);

      let foodshopping = null;
      for(const [fkey, fvalue] of modularresult.shopping.foodShoppingMap.entries()) {
        if(!shoppingmap.has(fkey)) {
          foodshopping = new FoodShopping();
          shoppingmap.set(fkey, foodshopping);
          foodshopping = shoppingmap.get(fkey);
          foodshopping.food = fvalue.food;
        }
        else
          foodshopping = shoppingmap.get(fkey);

        foodshopping.quantity += fvalue.quantity;
        foodshopping.volumen += fvalue.volumen;
      }
    ...
    for (const [key, value] of shoppingmap.entries()) {
      value.volumen = value.volumen.toFixed(2);
    }

    return {modular : modularmap, package: packagemap, shopping : shoppingmap};
  }
...
module.exports = {Modular, Coordinate, Position, Item, 
  ...
  FoodPresentation, FoodRequestPresentation, Package, FoodPackage, PackagePresentation}
